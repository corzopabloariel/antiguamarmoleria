...
Pyrus = function (e = null, dataPYRUS = null, modify = null) {
    ...
    this.formulario = ( id = "" , multiple = null ) => {
        ...
        try {
            ...
            ARR_form.forEach( function( rowElements ) {
                let TEXT_row = '<div class="row justify-content-center">';
    ...
    this.inputAdecuado = ( Object_ , element_name , id_name , STR_class , OBJ_funcion , placeholder , multiple_name = null , e ) => {
        ...
        if( Object_.VISIBILIDAD == 'TP_VISIBLE' || Object_.VISIBILIDAD == 'TP_VISIBLE_FORM' ) {
            switch( Object_.TIPO ) {
                case 'TP_LIST':
                    return this.listDatails(Object_, names, STR_class,OBJ_funcion, placeholder);
                case 'TP_ENTERO':
                ...
                case 'TP_IMAGE':
                    return this.inputImage(Object_, names, STR_class, OBJ_funcion);
                case 'TP_FILE':
    ...
    //-----------
    this.inputImage = (Object_, names, STR_class, OBJ_funcion = null) => {
        ...
        attr_aux += "<ul class='image--wh'>";
        if (Object_.WIDTH) {
            attr_aux += `<li>Ancho [${Object_.WIDTH}]</li>`;
            image.style.width = Object_.WIDTH;
        }
        if (Object_.HEIGHT) {
            attr_aux += `<li>Alto [${Object_.HEIGHT}]</li>`;
            image.style.height = Object_.HEIGHT;
        }
        attr_aux += "</ul>";
        if (Object_.EXT) {
            attr_aux += "<ul class='image--wh'>";
                attr_aux += `<li><strong>Extensiones permitidas:</strong> ${Object_.EXT}</li>`;
            attr_aux += "</ul>";
        }
        attr.innerHTML = attr_aux;
        ...
        if (Object_.HELP) {
            ...
            help.classList("form-text", "text-muted");
            help.innerHTML = Object_.HELP;
        }
        ...
        return attr.outerHTML + image.outerHTML + aviso.outerHTML + contenedorImage.outerHTML;
    };
    //-----------
    ...
    this.inputString = ( Object_ , names , STR_type , STR_class , OBJ_funcion = null , placeholder = "" ) => {
        ...
        if(STR_type == "file") {
            ...
            if( Object_.TIPO == "TP_IMAGE" || Object_.TIPO == "TP_BLOB" ) {
                ...
                if(Object_.SIMPLE === undefined) {
                    img = `<div class="d-flex overflow-hidden flex-wrap mx-auto" style="max-width: 100%; width: calc(${w} * 2); height: ${h};">`;
                        img += `<img id="src-${Arr.idElementForm}" class="mx-auto d-block border h-100 bg-dark" style="width: ${w};" src="" onError="this.src='${src}'" />`;
                        img += `<img id="src-${Arr.idElementForm}-2" class="w-100 mx-auto d-block border h-100 bg-ligth" style="width: ${w} ;max-width: 50%;" src="" onError="this.src='${src}'" />`;
                    img += `</div>`
                } else {
                    img = `<div class="d-flex overflow-hidden flex-wrap mx-auto" style="max-width: 100%; width: calc(${w} * 2); height: calc(${h} * 2);">`;
                        img += `<img id="src-${Arr.idElementForm}" class="mx-auto d-block border h-100 w-100 bg-dark" src="" onError="this.src='${src}'" />`;
                    img += `</div>`
                }
                ...
                input = `<div class="input-group">` +
                            ...
                            `<input type="hidden" name="${Arr.nameURLForm}" id="${Arr.idURLForm}" class="imgURL" value="" />` +
                            `<div class="input-group-append"><button style="height: calc(1.5em + 0.75rem + 2px);" disabled onclick="removeFile( this )" class="btn btn-danger btn-sm" id="${Arr.buttonElementForm}" type="button"><i class="fas fa-times-circle"></i></button></div></div><div class="input-group-text flex-column flex-wrap bg-white text-left"></div>`;
            }
        }
        if( Object_.HELP !== undefined )
            input += `<small class="form-text text-muted">${Object_.HELP}</small>`
