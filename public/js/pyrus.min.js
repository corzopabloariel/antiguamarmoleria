...
Pyrus = function (e = null, dataPYRUS = null, modify = null) {
            ...
            if (this.objeto.IDIOMAS) {
                if (this.objeto.IDIOMAS[x]) {
                    for (let y in this.objeto.IDIOMAS[x].ELEMENT) {
                        ...
                        CKEDITOR.replace(`${Arr.idElementForm}`, this.objeto.EDITOR[x]);
                    }
                    continue;
                }
            }
    ...
    this.constructorNames = ( names , addName = null ) => {
        ...
        if( names.idioma !== undefined ) {
            Arr.nameElementForm += `[${names.idioma}]`;
        ...
        if( addName !== null ) {
            ...
            if( names.idioma !== undefined ) {
                Arr[ `${addName}NameElementForm`] += `[${names.idioma}]`;
        ...
        if( names.multiple !== null ) {
            if( addName !== null ) {
                Arr[ `${addName}NameElementForm`] = `${this.name}[${names.multiple}][${names.element}]`;
            ...
            Arr.nameElementForm = `${this.name}[${names.multiple}][${names.element}]`;
            Arr.idElementForm = `${this.name}_${names.multiple}_${names.element}`;

            Arr.nameURLForm = `${this.name}[${names.multiple}][${names.element}][URL]`;
        ...
        if( names.id !== null ) {
            if( names.id != "" ) {
                ...
                if( addName !== null ) {
                    Arr[ `${addName}NameElementForm`] += `[${addName}][]`;
    ...
    this.inputImage = (Object_, names, OBJ_funcion) => {
        ...
        let Arr = this.constructorNames(names, "check");
        ...
        if (Object_.WIDTH) {
            ...
            attr_html += `<li>Ancho [cualquier medida]</li>`;
        if (Object_.HEIGHT) {
            attr_html += `<li>Alto [${Object_.HEIGHT}]</li>`;
            image.style.height = Object_.HEIGHT;
        } else
            attr_html += `<li>Alto [cualquier medida]</li>`;
        attr_html += "</ul>";
        ...
        if (Object_.FOLDER)
            attr_html += `<ul class='image--wh'><li><strong>Carpeta de guardado:</strong> ${Object_.FOLDER}</li></ul>`;
        if (Object_.EXT)
    ...
    this.inputString = (Object_, names, STR_type, OBJ_funcion = null, placeholder = "") => {
        ...
        let help = Object_.HELP ? this.help(Object_.HELP) : "";
        let STR_funcion = this.constructorFunction(OBJ_funcion, Arr.idElementForm);
		if (Object_.NECESARIO)
            element.required = true;
        if (Object_.DISABLED)
            element.disabled = true;
    ...
    //-----------
    this.check = (OBJ_elemento, names, OBJ_funcion = null) => {
    ...
    this.listDatails = (OBJ_elemento, names, STR_class, OBJ_funcion = null , placeholder = "" ) => {
        ...
        let label = OBJ_elemento.LABEL ? this.label(Arr.idElementForm, OBJ_elemento.NOMBRE) : "";
        let help = OBJ_elemento.HELP ? this.help(OBJ_elemento.HELP) : "";
        if (OBJ_elemento.NECESARIO)
            element.required = true;
        if (OBJ_elemento.DISABLED)
            element.disabled = true;
        if (OBJ_elemento.CLASS)
            element.classList.add(...OBJ_elemento.CLASS.split(" "));
